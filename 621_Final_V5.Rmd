...
Pasha, Fatima, Dogar, & Shahzad (2017) in their research explore the predictive accuracy of six algorithms for default prediction - linear discriminant analysis, Naïve Bayes, C4.5 decision tree, Logistic Regression, Neural Networks -MLP, and k-Nearest Neighbor (KNN).  Their work evaluates performance using metrics such as accuracy (correct classification, incorrect classification, precision, and recall.  The results show that the relatively newer algorithm of Multilayer Perceptron within the class of neural networks proves to be the best algorithm with an accuracy rate of 81.7%.  It is worth noting that logistic regression is a close second in performance at 81%.
...
Altabrawee (2016) examines fourteen classification models and their predictive accuracy in default prediction.  He notes the importance of a successful model's ability to avoid underfitting or overfitting training data.   In particular, the appropriate amount of regularization in the model should be used to limit under or overfitting.   Performance results are evaluated in terms of average accuracy, precision, recall, and F score.  The partial decision tree algorithm PART is determined to be the most accurate while Naïve Bayes-related algorithms are at the bottom in performance.
...
__TODO: description of modeling process__


#### Performance Measurement

A critical feature of the credit card default prediction models being evaluated is their ability accurately predict whether a cardholder will default on next month's payment.  As such, accuracy will be a measured used to determine a model's performance. Precision and recall will be used to provide further insight into the number of false positives and the number of false negatives, respectivley.

F1-score, or harmonic mean of the precision and recal, will used in addition to the kappa statistic.

accuracy, precision, recall, F score, kappa, False Positive Rate,
MAE
RMSE
...
Model selection is peformed using the performance metrics noted above as the selection criteria.   
...
### Binary Logistic Regression Model Using Stepwise Variable Selection
...
### Ridge and Lasso Regularization Generalized Linear


Ridge regression, also called L2 Regularization, attempts to avoid overfitting
...
# boruta_output <- Boruta(target ~ ., data=na.omit(train_all), doTrace=2)  # perform Boruta search
# 
# # 
...
cv.ridge <- cv.glmnet(x, y, family='binomial', alpha=0,  standardize=TRUE, type.measure='mse')  
...
ridge_model <- cv.ridge$glmnet.fit
summary(ridge_model)

#get test data
#x_test <- model.matrix(target~.,test_all)
x_test <- as.matrix(test_all[,-23]) # Removes class
#predict class, type="class"

ridge_prob <- predict(cv.ridge, newx = x_test, s=cv.ridge$lambda.min,type="response")

#translate probabilities to predictions
ridge_predict <- rep(0,nrow(test_all))
ridge_predict[ridge_prob>.5] <- 1
#confusion matrix
table(pred=ridge_predict,true=test_all$target)

fourfoldplot(table(pred=ridge_predict,true=test_all$target))
#accuracy
mean(ridge_predict==test_all$target)

```

### LASSO

Lasso regression, also called L1 Regularization, attempts to avoid overfitting
...
lasso_prob <- predict(cv.lasso,newx = x_test,s=cv.lasso$lambda.min,type="response")

#translate probabilities to predictions
lasso_predict <- rep(0,nrow(test_all))
lasso_predict[lasso_prob>.5] <- 1
#confusion matrix
table(pred=lasso_predict,true=test_all$target)

fourfoldplot(table(pred=lasso_predict,true=test_all$target))
#accuracy
mean(lasso_predict==test_all$target)
...
# grow tree
m1 <- rpart(target ~ .,  data = train_all, method = 'anova')
m1
...
#Normalize dataset
maxValue <- apply(cc_data, 2, max)  #CreditCardnn
minValue <- apply(cc_data, 2, min)   #CreditCardnn


CreditCardnn <- as.data.frame(scale(cc_data, center = minValue, scale = maxValue - minValue))

kable(print(head(cc_data, 2)))

#Rename to target variable
#colnames(CreditCardnn)[25] <- "target"

#CreditCardnn <- dplyr::rename(CreditCardnn, target=DEFAULT)
...
```{r}
DataPred <- neuralnet::compute(neuralModel, test_nn[, -1])

#DataPred$net.result

#translate probabilities to predictions --> 
# KF: Not sure this is the correct to get probability .  Compure returns a

nn_predict <- rep(0,nrow(test_nn))
nn_predict[DataPred$net.result>.5] <- 1
#confusion matrix
table(pred=nn_predict,true=test_nn$target)

fourfoldplot(table(pred=nn_predict,true=test_nn$target))
#accuracy
mean(nn_predict==test_nn$target)

```

##Naive Bayes Classifier  

Since we predicting a categorical response, we will build a naive bayes model although not all the features are categorical.  
...
nb_fold = trainControl(method='cv',number=10)


# From Caret/klaR package
nb_model_c = train(target ~.,data = train_all, method='nb',trControl=nb_fold)

#Prediction
nb_predict_train_c = predict(nb_model_c, train_all, type = 'raw')
#nb_predict_train_e=predict(nb_model_e, newdata=train_all, type="class")

#Confusion matrix to check accuracy - caret package
cfm_c <- confusionMatrix(nb_predict_train_c, train_all$target, positive = '1', dnn = c("Prediction", "Actual"))

```

The Naive Bayes model performed relatively well, the accuracy on the trainning data set, using 10-folds Cross Validation is only 79%% and the Kappa = 0.168. The model is predicting a higher number of false 
* On reasaon may be due to we have a very high number of false negative, where the model predicted failed to predict loaon default when in fact there was one. For the business case of a bank trying to identify potential at risk cusotmers, this results is detrimental to the model, even with an accuracy of 79%.  

We will try to refine our model to get better results. 

One reason may due to some corelation between the predictors, Naive Bayes models have an assumption of indepence. 
Another reason, may due to the low probability of a default occurring.  

We will consider the possible corelation between some of the predictors, we will consider a threshold of 0.75 (in absolute value) and remove any predictors that have correlation equal or superior to threshold.

```{r corralation nb, echo=FALSE, message=FALSE, warning=FALSE}
train_predictors <- train_all
train_predictors$target <- NULL

corr_limit <- 0.75
predcorr = cor(train_predictors)
highCorrpp <- findCorrelation(predcorr, cutoff = corr_limit, verbose = FALSE)
train_all_nb <- train_all[, -highCorrpp]

```

We have 3 predictors that have such high corelations; PAY_2, PAY_3, PAY4, we will remove them and re-tain our model.  

```{r Naive Bayes2, echo=FALSE, message=FALSE, warning=FALSE}

# From Caret/klaR package
nb_model_c2 = train(target ~.,data = train_all_nb, method='nb',trControl=nb_fold)

#Prediction
nb_predict_train_c2 = predict(nb_model_c2, train_all_nb, type = 'raw')
#nb_predict_train_e=predict(nb_model_e, newdata=train_all, type="class")

#Confusion matrix to check accuracy - caret package
cfm_c2 <- confusionMatrix(nb_predict_train_c, train_all_nb$target, positive = '1', dnn = c("Prediction", "Actual"))

```

Dropping these predictors did not improve the model.  

Let us now consider the probability of a default in our current tainning data set.  

```{r Probabilities nb, echo=FALSE, message=FALSE, warning=FALSE}

df <- train_all %>% dplyr::group_by(target) %>% summarise(count=n())
df$ratio <- round(df$count/nrow(train_all), 4)

knitr::kable(df)
``` 

The ratio to default to non-default is about 3 to 1, this parsity of the data may be the reason why the naive bayes model does not performed as well as the other.  
...
```{r nnet-caret}
c.fit <- train(as.factor(target) ~ ., data = train_nn,
         method = "nnet", maxit = 1,   #increase later since this takes times to run 
         trace = F) 
    
    
nnet.predict <- predict(c.fit, newdata = test_nn[,-23], type='raw')
#prestige.rmse <- sqrt(mean((prestige.predict - prestige.test$income)^2)

confusionMatrix(nnet.predict, as.factor(test_nn[,23]))    

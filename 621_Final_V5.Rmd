...
calc_metrics <- function(model_name, 
  ...
  compare <- cbind (actual=test$target, y_pred_model)  # combine
...
calc_metrics_2 <- function(model_name, 
  ...
  compare <- cbind (actual=y_test, y_pred_model)  # combine
...
calc_metrics_3 <- function(model_name, 
  ...
  #y_pred_model      <- factor( ifelse(pred_model > 0.5, 1, 0) )

  compare <- cbind (actual=target, y_pred_model)  # combine
...
Ridge regression, also called L2 Regularization, attempts to avoid overfitting by..
...
kable(m3ridge_metrics[[1]])
kable(m4lasso_metrics[[1]])
